...
> **フロントエンド資産について**：CSSはTailwindをローカルビルドし、ビルド済み成果物
> （`public/build/`）をリポジトリに同梱しています。Xserver共有サーバーにNode.jsは不要です。
> デザインを変更した場合のみ、手元で `npm install && npm run build` し直して再デプロイしてください。
